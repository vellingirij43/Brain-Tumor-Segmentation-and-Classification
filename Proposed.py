...
# Improved Flow Direction Algorithm (FDA) Starting to ending line No. (61 to 68)

def Proposed(alpha, fobj, lb, ub, maxiter):
        ...
        for i in range(len(alpha)):
            ...
            for j in range(N):
                ...
                if fitness_neighbor < fitness_flow[i]:
                    ...
                    bestfit = min(fitness_neighbor)
                    worstfit = max(fitness_neighbor)
                    r = bestfit / worstfit * 0.2
                    # r = np.random.rand()
